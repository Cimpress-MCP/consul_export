...
var preprocess_results = function(results) {
  ...
  var services = Object.getOwnPropertyNames(results.services);
  services.forEach(function(service) {
    var nodes_for_service = results.services[service];
...
var render_results = function(results) {
  config.dns.outputters.forEach(function(outputter) {
    require('./dns/output/' + outputter.type + '.js')(outputter)(results, function(err) {
      ...
      // If there's a configured backup for this outputter, process it now that we know
      // we have valid results.
      if (outputter.backup) {
        backup.populate(outputter, results);
      }
...
var gather_consul_data = function(cb) {
  request(util.format('http://%s:%s/v1/catalog/nodes', config.consul.host, config.consul.port), function(err, res, body) {
    if (err) return cb('Error reading /catalog/nodes/: ' + err);

    var body = JSON.parse(body);

    var results = {'nodes': body, 'services':{}};
